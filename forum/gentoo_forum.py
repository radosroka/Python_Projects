...
def get_num_of_topics():
	...
	table = soup.find("table", { "border":"0", "cellpadding":"4", "cellspacing":"1", "class":"forumline", "width":"100%"})
...
def parse_posts(html):
	...
	rows1 = soup.find_all( "td", {"class":"row1", "width":"100%", "height":"28", "valign":"top"} )
	rows2 = soup.find_all( "td", {"class":"row2", "width":"100%", "height":"28", "valign":"top"} )
...
def init_mode(thread_name, start, end):
	...
	while 1:
		...
		soup = None
		while 1:
			response = urllib2.urlopen(forum_link + "viewtopic-t-" + str(index) + "-postdays-0-postorder-asc-start-" + str(page) + ".html")
			html = response.read()

			if page == 0 and "The topic or post you requested does not exist" in html:
				break
			...
			print thread_name + " : " +forum_link + "viewtopic-t-" + str(index) + "-postdays-0-postorder-asc-start-" + str(page) + ".html"
			soup = BeautifulSoup(html)
		...
		if soup:
			...
			topic_address.text = forum_link + "viewtopic-t-" + str(index) + "-postdays-0-postorder-asc-start-" + str(page) + ".html"
